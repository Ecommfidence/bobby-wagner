...
theme.Variants = (function() {
  function Variants(options) {
    ...
    this._enforceStyleOptions();
    this._enforceColorSizeOptions();
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _enforceStyleOptions: function() {
      ...
      {
      // Variants that are part of the current style choice
      var found = _.filter(variants, function(variant) {
        return variant.options[0] == options[0];
      });
      
      // Update the style with selected Style title
      $('.product-single__style').html(options[0]);
      
      // Go through color and size
      _.each([1,2], function(option_index){
        var found_options = _.flatMap(found, function(variant){ return variant.options[option_index]; });
        var found_options_uniq = _.uniq(found_options);
        var found_options_instock = _.flatMap(found, function(variant){ if(variant.available) return variant.options[option_index]; });
        var found_options_instock_uniq = _.uniq(_.compact(found_options_instock));

        //console.log("UNIQUE "+option_index+":", found_options_uniq);
        //console.log("UNIQUE IN STOCK "+option_index+":", found_options_instock_uniq);

        if(found_options_uniq.length == 1){
          $('.swatch[data-option-index="'+option_index+'"]').hide();
        }
        else{
          $('.swatch[data-option-index="'+option_index+'"]').show();
        }

        $('#SingleOptionSelector-'+option_index+' option').each(function(index, element){
          var elem = $(element);
          if(_.includes(found_options_uniq, elem.val())){
            elem.removeAttr("disabled");
          }
          else{
            if($("#SingleOptionSelector-"+option_index).val() == elem.val()){
              //console.log("TRIGGER ON "+found_options_uniq[0]); 
              $("#SingleOptionSelector-"+option_index).val(found_options_uniq[0]).trigger('change');
            }
            elem.attr("disabled", "disabled");
          }
        });

        $('.swatch[data-option-index="'+option_index+'"] .swatch-element').each(function(index, element){
          var elem = $(element);
          
          //console.log('MATCHING: ', found_options_uniq, $(element).data('value'));
          if(_.includes(found_options_uniq, elem.data('value'))){
            //console.log("match");
            elem.show();
          }
          else{
      ...
      // Update Available As Section
      $('.product-option').each(function(index, element){
        if(options[0] == $(element).data('option-value'))
      ...
      $('.product-single__thumbnails-item').each(function(index, element){
        $(element).hide();
        _.each(variants, function(variant){
          //console.log(variant);
          var currentImage = theme.Images.getSizedImageUrl(variant.featured_image.src, '160x').split('?')[0];
          if(variant.options[0] == options[0] && currentImage == $(element).find('img').attr('src').split('?')[0]){
            $(element).show();
          }
        });
      ...
      //theme.Product._destroyThumbnailSlider();
      //theme.Product._initThumbnailSlider();
      
    },
    
    _enforceColorSizeOptions: function(){
      var options = this._getCurrentOptions();
      var variants = this.product.variants;
      ...
      _.each([1,2], function(option_index){
        ...
        $('.swatch[data-option-index="'+option_index+'"] .swatch-element').each(function(index, element){
          ...
          var found_instock = _.filter(variants, function(variant) {
            //console.log("TEST",  variant.available, variant.options[0] == options[0], 
            //            variant.options[((option_index+2)%2)+1] == options[((option_index+2)%2)+1],
            //            variant.options[0], options[0], variant.options[((option_index+2)%2)+1], options[((option_index+2)%2)+1]);
            return variant.available && 
              variant.options[0] == options[0] && 
              variant.options[((option_index+2)%2)+1] == options[((option_index+2)%2)+1];
          });
          var found_options_instock = _.flatMap(found_instock, function(variant){ return variant.options[option_index]; });
          var found_options_instock_uniq = _.uniq(_.compact(found_options_instock));
          
          //console.log("Found Instock", found_instock, found_options_instock_uniq);
          
          if(_.includes(found_options_instock_uniq, elem.data('value'))){
            //console.log("INSTOCK");
          	elem.removeClass('soldout').addClass('available').find(':radio').removeAttr('disabled')
          }
          else{
            //console.log("SOLD OUT");
            elem.addClass('soldout').removeClass('available').find(':radio').attr('disabled', 'disabled');
          }
        });
      });
    },
    ...
    _onSelectChange: function(event) {
      if(event.target.id == "SingleOptionSelector-0"){
      	this._enforceStyleOptions();
      }
      this._enforceColorSizeOptions();
    ...
    _updateSwatches: function(variant){
      if (variant) {
        for (var i=0,length=variant.options.length; i<length; i++) {
          var radioButton = this.$container.find('.swatch[data-option-index="' + i + '"] :radio[value="' + variant.options[i] +'"]');
          if (radioButton.size()) {
    ...
    _updateMasterSelect: function(variant) {
      ...
      $originalSelector.val(variant.id).trigger('change');
    }
  });
...
jQuery(function() {
  ...
  jQuery('.product-option').click(function() {
    jQuery('.product-form')
      .find('.single-option-selector')
      .eq(jQuery(this).attr('data-option-index'))
      .val(jQuery(this).attr('data-option-value'))
      .trigger('change');
  });
});
